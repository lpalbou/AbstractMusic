...
class AceStepConditionGenerationModel(AceStepPreTrainedModel):
    ...
    def get_init_context(cls, *args, **kwargs):
        """
        Transformers v4/v5 may initialize models on the `meta` device by default in
        `from_pretrained()` to reduce peak CPU memory. (v4 via `init_empty_weights`,
        v5 via `torch.device("meta")`.)

        ACE-Step v1.5 initializes its audio quantizer during `__init__` and
        performs Python `assert` checks / scalar extraction that attempt to
        materialize tensor values. This fails on `meta` tensors (e.g.
        `Tensor.item()` is not supported).

        We therefore override the init context to **avoid meta initialization**
        for this model.
        """

        ctxs = super().get_init_context(*args, **kwargs)
        out = []
        for c in ctxs:
            # Transformers v5: `torch.device("meta")` is included directly.
            try:
                if isinstance(c, torch.device) and c.type == "meta":
                    continue
            except Exception:
                pass
            # Transformers v4: `init_empty_weights()` is a generator contextmanager.
            # It exposes the underlying generator function via `.func`.
            try:
                func = getattr(c, "func", None)
                if func is not None and getattr(func, "__name__", "") == "init_empty_weights":
                    continue
            except Exception:
                pass
            out.append(c)
    ...
    def prepare_condition(
        ...
        # IMPORTANT: when no sample is marked as "cover", hints are not needed and we skip
        # tokenize/detokenize entirely. This avoids unnecessary compute and sidesteps
        # MPS mixed-dtype kernel issues seen in tokenizer paths.
        need_cover_hints = False
        if is_covers is not None:
            try:
                need_cover_hints = bool((is_covers > 0).any().item())
            except Exception:
                need_cover_hints = True
        else:
            is_covers = torch.zeros((src_latents.shape[0],), device=src_latents.device, dtype=src_latents.dtype)

        if not need_cover_hints:
            lm_hints_25Hz = src_latents
        elif precomputed_lm_hints_25Hz is not None:
